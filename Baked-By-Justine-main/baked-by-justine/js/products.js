...
async function loadCategories() {
  try {
    ...
    const fallbackCategories = (typeof STATIC_PRODUCTS !== 'undefined' && Array.isArray(STATIC_PRODUCTS))
      ? [...new Set(STATIC_PRODUCTS.map(p => p.category).filter(Boolean))]
      : [];
...
async function loadProducts() {
  ...
  try {
    ...
    if (typeof STATIC_PRODUCTS !== 'undefined' && Array.isArray(STATIC_PRODUCTS)) {
      allProducts = STATIC_PRODUCTS.map(function(p) {
        p.discounted_price = p.discount_percent > 0 ? Math.round(p.price * (1 - p.discount_percent / 100) * 100) / 100 : null;
        return p;
      });
      renderProducts();
    } else {
